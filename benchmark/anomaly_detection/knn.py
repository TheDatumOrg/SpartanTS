...
class KNN:
    ...
    """
    def __init__(self, contamination=0.1, n_neighbors=10, method='largest',
...
def run_KNN(data, symbol_data, slidingWindow=24, n_neighbors=10, method='largest', n_jobs=1, metric='minkowski'):
    clf = KNN(n_neighbors=n_neighbors, method=method, n_jobs=n_jobs, metric=metric)
    clf.fit(symbol_data)
    ...
    if score.shape[0] < n_samples:
        left_padding = math.ceil((slidingWindow-1)/2)
        right_padding = n_samples - score.shape[0] -left_padding
        score = np.array([score[0]]*left_padding + 
                    list(score) + [score[-1]]*right_padding)
