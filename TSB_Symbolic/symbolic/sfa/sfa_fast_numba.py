#Adapted from sktime _sfa_fast_numba
...
@njit(fastmath=True,cache=True)
def _inverse_transform(dfts,reconstruction_length):
    n_instances,num_windows_per_inst,dft_len = dfts.shape
    

    X_reconstruction = np.zeros((n_instances,num_windows_per_inst,reconstruction_length))
    for i in prange(n_instances):
        with objmode(X_inv_fft="float64[:,:]"):
            X_inv_fft = np.fft.irfft(dfts[i],n=reconstruction_length,axis=1)
        X_reconstruction[i,:] = X_inv_fft
