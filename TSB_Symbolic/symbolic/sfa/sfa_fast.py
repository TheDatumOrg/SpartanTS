...
class SFAFast():
    ...
    def fit_transform(self, X, y=None):
        ...
        from .sfa_fast_numba import _transform_case,remove_repeating_words
        ...
        if self.window_size == 0:
            ...
            self.window_size = int(X.shape[1] *  self.window_size)

        self.window_size = max(self.word_length, min(self.window_size, X.shape[1]))

        # print("window size: ", self.window_size)

        self.inverse_sqrt_win_size = (
            1.0 / math.sqrt(self.window_size) if not self.lower_bounding else 1.0
        )
        ...
        if self.window_size != X.shape[1]:
            ...
            self.word_length_actual = self.word_length
        self.dft_length = (
        ...
        if self.build_histogram and self.window_size != X.shape[1]:
    ...
    def transform(self, X, y=None):
        ...
        from .sfa_fast_numba import _transform_case,create_bag_transform
    ...
    def transform_to_bag(self, words, word_len, y=None):
        ...
        from .sfa_fast_numba import create_bag_feature_selection,create_bag_none,create_feature_names
        ...
        if self.feature_selection == "none" and (
            ...
            feature_names = set(np.arange(self.alphabet_size ** self.word_length))
            # print(feature_names)
            ...
            if self.feature_selection == "none":
                ...
                # relevant_features_idx = np.arange(self.alphabet_size ** self.word_length)
    ...
    def _binning(self, X, y=None):
        from .sfa_fast_numba import _binning_dft
    ...
    def _mcb(self, dft):
        breakpoints = np.zeros((self.word_length_actual, self.alphabet_size))
        mindist_breakpoints = np.zeros((self.word_length_actual,self.alphabet_size+1))
        mindist_breakpoints[0,:] = - sys.float_info.max
        # print(self.word_length_actual)
        dft = np.round(dft, 2)
        for letter in range(self.word_length_actual):
    ...
    def _shorten_bags(self, word_len, y):
        from sfa._sfa_fast_numba import shorten_words
    ...
    def inverse_transform(self,dfts,reconstruction_length):
        from sfa._sfa_fast_numba import _inverse_transform

        n_instances,num_windows_per_inst,dft_len = dfts.shape

        dft_len = dft_len // 2

        if reconstruction_length % 2 == 0:
            full_dft_len = int((reconstruction_length / 2) + 1)
        else:
            full_dft_len =int((reconstruction_length+1) / 2)

        reals = dfts[:,:,0::2]
        imag = dfts[:,:,1::2]
        dft_complex = reals + 1j*imag  
        dft_complex = dft_complex/reconstruction_length
        dft_padded = np.pad(dft_complex,((0,0),(0,0),(0,full_dft_len-dft_len)))

        X_reconstructions = _inverse_transform(dft_padded,reconstruction_length=reconstruction_length)
